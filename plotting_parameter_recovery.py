...
df_plot = pd.read_csv('parameter_recovery200_AR1.csv')
#df_plot = pd.read_csv('parameter_recovery200_AR99.csv')
#df_plot = pd.read_csv('parameter_recovery200_AR95.csv')
#df_plot = pd.read_csv('parameter_recovery500_AR1.csv')
...
def scatter(x,y,axis):
    p = sns.scatterplot(data=df_plot, x=x, y=y,hue = "ntrials",legend = False,x_jitter=.1,ax=ax[axis])
...
def my_scatter(x,y):
    p = sns.FacetGrid(df_plot, col="ntrials")
    p.map(sns.scatterplot, x, y, alpha=.7)
    
    # find the min and max value to determine the range of x and y axis
    low = min(df_plot[x] + df_plot[y])
    high = max(df_plot[x] + df_plot[y])
