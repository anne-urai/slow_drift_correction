...
data <- read_csv("param_recovery_prevrespconf_AR_noSys_99.csv")

df <- data
#df <- data[data$AR_fit > .8,] 
...
p_AR <- ggplot(df, aes(x=as.factor(ntrials), y=AR_fit, col = as.factor(ntrials))) +
  geom_hline(yintercept=df$AR_sim) +
  geom_point(position = position_dodge(1)) +
  theme_bw() +
  #ylim(c(-1,1)) +
  stat_summary(geom = "point",fun = "mean",col = "black",size = 3,shape = 24,fill = "red") +
  labs(x = "Number of trials", y = "AR") +
  theme(legend.position="none")


p_C <- ggplot(df, aes(x=AR_fit, y=sigma_fit)) +
  geom_hline(yintercept=df$sigma_sim) +
  geom_point(col="blue") +
  #ylim(c(0,.75)) +
...
grid.arrange(p_sens,p_resp,p_conf,p_respconf,p_sign,p_absevi,p_signabsevi,p_sigma,p_AR,p_C, ncol = 3)
dev.off()
