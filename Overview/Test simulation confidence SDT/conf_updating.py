...
inputs = np.ones((ntrials, 6)) #evidence, prevresp, prevconf, prevrespconf, used in the model
...
prevresp, prevconf, prevrespconf = 0, 0, 0 # initialize for first trial
for i in range(ntrials):
    ...
    crit = w_prevresp*prevresp + w_prevconf*prevconf + w_prevrespconf*prevrespconf + bias + drift[i]
